...
The rule is applied on the canvas. **It is not yet applied on the works grid or the project pages**, which still crop media into composed shapes (16:9 / 3:4 / 4:3 / 1:1 by grid position on `works.html`; wide / tall / square on `work.html`). Doing so there needs a decision first: a 9:16 clip at its native ratio in a seven-column card would be ~1400px tall. Open with Frank. Adding or editing a project is editing this file; no new HTML per project. Current entries are placeholders clearly marked `PLACEHOLDER` — real content (Frank's actual projects, images/video, headshot) has not been delivered yet as of this repo's creation.
