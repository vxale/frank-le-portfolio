...
- **No connecting lines.** Edges between short node labels read as a diagram; edges between paragraphs read as a flowchart of prose. Order is carried by the numbers.
- Passage text moved from `--step-1` to `--step-2`. This is the body copy of the page and it was set at control size.

**Two columns (Sept 12 2026).** The chain was a single column with each passage indented a step further than the last. Frank asked for two columns instead. It is CSS multi-column (`column-count: 2` on `.beats`), not a two-column grid, for two reasons: multicol balances the columns by height on its own, and a numbered sequence that flows 01–02–03 down the left and 04–05 down the right is how columns of prose are read — a grid would have paired 01 with 02 on one row and left a hole under whichever ran shorter. `break-inside: avoid` keeps each passage whole. The stagger is gone (an indent that climbed with `nth-child` would have shoved the whole right-hand column six rem in), and `.beats` grew from 8 to 9 grid columns so each measure lands at ~50 characters rather than 43. One column again under 860px. Folding still works inside the multicol box — the fold shrinks its passage and the columns rebalance.
