...
**Two columns, one staircase (Sept 12 2026).** Frank asked for two columns, then for the descending indent back on top of them. It is CSS multi-column (`column-count: 2` on `.beats`), not a two-column grid: multicol balances the columns by height on its own, and a numbered sequence that flows 01–02 down the left and 03–04–05 down the right is how columns of prose are read — a grid would have paired 01 with 02 on one row and left a hole under whichever ran shorter. `break-inside: avoid` keeps each passage whole. One column again under 860px, and folding still works inside the multicol box.

The indent runs as **one continuous staircase through the whole sequence**, not one that restarts at the top of each column: 03 sits two steps in wherever it lands, 05 four. That is the only version that survives multicol — the break between columns moves with content and viewport, so a stagger that restarted per column would need to know where the break fell, and CSS can't. A continuous one doesn't care. The step (`--step` on `.beat`, ~22px at 1440) is about two-thirds of the single column's, capped at the fifth, because indent is subtracted from the measure; `.beats` grew from 8 to 10 grid columns to pay for it. Measured at 1440: indents 0/22/43/65/86px, measures 56 down to 46 characters. On a phone `--step` drops to 0.5rem so the staircase is there in miniature rather than eating an eighth of the width.
...
- **Native ratio on the project page.** The template forced the lead frame to 16:9, which on a 9:16 bridal film would cut off the top and bottom third of every dress. Frames with `width`/`height` now render as `.shot--native`; portrait ones are height-capped (`.floater--upright`) so a 9:16 clip is ~395×700 in the lead spot rather than 1700px tall. **The works grid followed the same day**, once Frank saw Vision of Love cropped there: `.media-frame--native` / `.media-frame--upright`, height cap `min(70vh, 48rem)`, landscape at full card width. The rule now applies everywhere media is shown; the position-based crops remain only for placeholder entries.
